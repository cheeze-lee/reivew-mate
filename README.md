...
- 내장 에이전트가 질문/선택코드 기반으로 리포 전체를 자동 탐색해 PR 외 관련 파일 컨텍스트를 일부 수집
...
4. 필요에 따라 컨텍스트 토글 조정
   ...
   - `전체 리포 탐색`은 내장 에이전트가 질문/선택코드에서 검색어를 추출해 자동 수행
...
- 별도 ON/OFF 없이, 질문 전송 시 내장 에이전트가 자동으로 검색어를 추출해 GitHub 코드검색으로 PR 외 파일 후보를 찾습니다.
- 수집 제한(기본):
  - 검색어 최대 6개
  - 검색 질의 최대 4회, 질의당 결과 최대 8개
  - 모델 컨텍스트에 포함하는 파일 최대 8개
  - 파일당 최대 12,000자, 총 90,000자
- 목적: 변경 diff만으로 보이지 않는 선언/호출/구조 맥락을 보강
- 주의: 추가 파일이 외부 LLM API로 전송되므로, 민감 코드가 있는 저장소에서는 신중히 사용하세요.
